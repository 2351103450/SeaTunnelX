...
- [ ] `observability.enabled=true` 时：
  - [ ] `app.external_url` 非空且为 `http/https`；
  - [ ] `observability.prometheus.url` / `alertmanager.url` / `grafana.url` 为合法 URL。
- [ ] 校验失败 fail-fast（启动失败并打印明确错误）。

### 3.3 HTTP SD 接口

- [ ] 新增 handler：`GET /api/v1/monitoring/prometheus/discovery`
- [ ] 返回 Prometheus HTTP SD 标准数组：
  - `targets`: `host:port`
  - `labels`: `cluster_id` / `cluster_name` / `env`
- [ ] 仅返回可探测通过的 metrics 目标（MVP 可按现有探测逻辑）。

### 3.4 Alertmanager Webhook

- [ ] 新增 handler：`POST /api/v1/monitoring/alertmanager/webhook`
- [ ] 接收 Alertmanager webhook 标准 payload；
- [ ] 落库字段（MVP）：
...
- [ ] 去重策略：`fingerprint + starts_at` 唯一 upsert。

---

## 4. 验收标准（M1）

- [ ] `observability.enabled=true` 且 `app.external_url` 非法时，SeaTunnelX 启动失败并提示具体错误；
- [ ] Prometheus 请求 HTTP SD 接口可拿到标准 TargetGroup JSON；
- [ ] Alertmanager webhook 推送后，告警记录可在数据库中查询到，重复告警可正确 upsert；
- [ ] `observability.enabled=false` 时，不注册以上公开接口。
