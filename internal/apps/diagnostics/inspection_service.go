...
func (s *Service) evaluateOfflineNodeFindings(statusInfo *cluster.ClusterStatusInfo) []*ClusterInspectionFinding {
	...
	for _, node := range statusInfo.Nodes {
		...
		findings = append(findings, &ClusterInspectionFinding{
			...
			Summary:         fmt.Sprintf("Node %s (%s) is offline", resolveInspectionNodeName(node), strings.TrimSpace(string(node.Role))),
			Recommendation:  "Check host heartbeat, agent status, and SeaTunnel process state before retrying operations.",
	...
	for _, event := range events {
		...
		case monitor.EventTypeRestartFailed, monitor.EventTypeRestartLimitReached:
			...
			findings = append(findings, &ClusterInspectionFinding{
				...
				CheckName:       "Restart Failure",
				Summary:         fmt.Sprintf("Recent %s detected for process %s", strings.TrimSpace(string(event.EventType)), strings.TrimSpace(event.ProcessName)),
				Recommendation:  "Review node event history and runtime logs, then verify startup command and dependencies.",
...
func (s *Service) evaluateRecentErrorFindings(ctx context.Context, clusterID uint, lookbackWindow time.Duration, errorThreshold int) ([]*ClusterInspectionFinding, error) {
	...
	for _, burst := range bursts {
		...
		findings = append(findings, &ClusterInspectionFinding{
			...
			Summary:             fmt.Sprintf("Recent error group \"%s\" occurred %d times in the last %d minutes", group.Title, burst.RecentCount, int(lookbackWindow/time.Minute)),
			Recommendation:      "Open the error center to inspect grouped evidence and identify the underlying dependency or runtime issue.",
...
func (s *Service) evaluateActiveAlertFindings(ctx context.Context, clusterID uint) ([]*ClusterInspectionFinding, error) {
	...
	for _, alert := range alerts.Alerts {
		...
		finding := &ClusterInspectionFinding{
			...
			CheckName:       "Active Alert",
			Summary:         fmt.Sprintf("Active %s alert: %s", strings.TrimSpace(string(alert.Severity)), strings.TrimSpace(alert.AlertName)),
			Recommendation:  "Review the alert detail and linked runtime evidence before performing restart or scale actions.",
